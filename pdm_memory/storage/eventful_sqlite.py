...
class EventfulSQLiteDriver(SQLiteDriver):
    ...
    def save_source_event(
        ...
        Idempotent on ``(user, content_hash)`` — this is AC1. The second
        ingest of the same message attaches its signatures to the first
        event instead of writing the raw content again.
        """
    ...
    def find_event_by_hash(
        ...
        row = self._conn().execute(
            "SELECT * FROM pdm_source_events WHERE user = ? AND content_hash = ? LIMIT 1",
            (user, content_hash),
        ).fetchone()
        return event_from_row(row) if row else None
    ...
    def update_source_event(self, event_id: str, **fields: Any) -> None:
        ...
        raise AppendOnlyViolation(
            "pdm_source_events",
            "update",
            detail="Record a new event instead.",
        )
    ...
    def resolve_or_create_entity(
        ...
        *,
        ...
        row = conn.execute(
            ...
            WHERE user = ? AND canonical_norm = ? AND origin_field_id = ?
            LIMIT 1
            """,
            (user, norm, field_id),
        ).fetchone()
        ...
        taken = conn.execute(
            "SELECT COUNT(*) FROM pdm_entities WHERE user = ? AND canonical_norm = ?",
            (user, norm),
        ).fetchone()[0]
        ...
        entity = EntityRecord(
            ...
            disambiguator=self._free_disambiguator(conn, user, norm, field_id, taken),
...
def enable_events() -> None:
    """
    Make ``Memory(store="./x.db")`` event-aware for the rest of the process.
    ...
    logger.debug("[PDM-Events] SQLite scheme now resolves to EventfulSQLiteDriver")
